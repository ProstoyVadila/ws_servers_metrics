...
pub static WS_CONNECTIONS: Lazy<IntGauge> = Lazy::new(|| {
    IntGauge::new(
        "ws_connections",
        "an amount of ws connections to the server",
    )
...
pub static ALL_WS_CONNECTIONS_TOTAL: Lazy<IntCounter> = Lazy::new(|| {
    IntCounter::new(
        ...
        "a counter of all new connections to the server",
    )
...
pub static WS_CONN_CLOSED_ERRORS_TOTAL: Lazy<IntCounter> = Lazy::new(|| {
    IntCounter::new(
        ...
        "an amount of closed websocket connections",
    )
    .expect("Cannot create ws_conn_closed_errors_total")
});

pub static BUCKETS: [f64; 14] = [
    0.0001, 0.0005, 0.0025, 0.005, 0.0075, 0.01, 0.025, 0.05, 0.1, 0.25, 0.5, 1.0, 2.0, 5.0,
];
...
pub fn get_prometheus() -> PrometheusMetrics {
    ...
    prom.registry()
        .register(Box::new(ALL_WS_CONNECTIONS_TOTAL.clone()))
        .expect("Cannot register ws_connections_total metric");
    prom.registry()
        .register(Box::new(WS_CONN_CLOSED_ERRORS_TOTAL.clone()))
    ...
    prom.registry()
        .register(Box::new(WS_MESSAGE_HANDLING_DURATION_SECONDS.clone()))
